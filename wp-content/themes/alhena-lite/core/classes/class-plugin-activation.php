	...
	class TGM_Plugin_Activation {
		...
		protected function add_admin_menu( array $args ) {
			if ( has_filter( 'tgmpa_admin_menu_use_add_theme_page' ) ) {
				_deprecated_function( 'The "tgmpa_admin_menu_use_add_theme_page" filter', '2.5.0', esc_html__( 'Set the parent_slug config variable instead.', 'alhena-lite' ) );
			}
		...
		public function show_tgmpa_version() {
			echo '<p style="float: right; padding: 0em 1.5em 0.5em 0;"><strong><small>',
				esc_html( sprintf( _x( 'TGMPA v%s', '%s = version number', 'alhena-lite' ), self::TGMPA_VERSION ) ),
				'</small></strong></p>';
		}
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function get_plugin_status_text( $slug ) {
			...
			return sprintf(
				_x( '%1$s, %2$s', '%1$s = install status, %2$s = update status', 'alhena-lite' ),
				$install_status,
				$update_status
			);
		}
		...
		public function no_items() {
			printf( wp_kses_post( __( 'No plugins to install, update or activate. <a href="%1$s">Return to the Dashboard</a>', 'alhena-lite' ) ), esc_url( self_admin_url() ) );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		}
		...
		protected function get_row_actions( $item ) {
			...
			if ( ! $this->tgmpa->is_plugin_installed( $item['slug'] ) ) {
				$actions['install'] = _x( 'Install %2$s', '%2$s = plugin name in screen reader markup', 'alhena-lite' );
			} else {
				// Display the 'Update' action link if an update is available and WP complies with plugin minimum.
				if ( false !== $this->tgmpa->does_plugin_have_update( $item['slug'] ) && $this->tgmpa->can_plugin_update( $item['slug'] ) ) {
					$actions['update'] = _x( 'Update %2$s', '%2$s = plugin name in screen reader markup', 'alhena-lite' );
				}

				// Display the 'Activate' action link, but only if the plugin meets the minimum version.
				if ( $this->tgmpa->can_plugin_activate( $item['slug'] ) ) {
					$actions['activate'] = _x( 'Activate %2$s', '%2$s = plugin name in screen reader markup', 'alhena-lite' );
				}
			}
